...
const ConnectionsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/profile/getUser/${user}`);
  ...
  const handleAcceptConnection = async (senderEmail, index) => {
    try {
      ...
      const response = await axios.post('http://localhost:3000/profile/acceptConnection', payload);
